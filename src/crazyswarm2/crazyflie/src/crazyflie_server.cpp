...
public:
    ...
    , cfbc_(cfbc)
  {
    ...
    subscription_cmd_vel_world_ = node->create_subscription<geometry_msgs::msg::Twist>(name + "/cmd_vel_world", rclcpp::SystemDefaultsQoS(), std::bind(&CrazyflieROS::cmd_vel_world_changed, this, _1), sub_opt_cf_cmd);
    subscription_cmd_vel_hover_ = node->create_subscription<geometry_msgs::msg::Twist>(name + "/cmd_vel_hover", rclcpp::SystemDefaultsQoS(), std::bind(&CrazyflieROS::cmd_vel_hover_changed, this, _1), sub_opt_cf_cmd);
...
private:
  ...
  void cmd_vel_hover_changed(const geometry_msgs::msg::Twist::SharedPtr msg)
  {
    float vx = msg->linear.x;
    float vy = - (msg->linear.y);
    float height = msg->linear.z;
    float yawrate = msg->angular.z;
    // RCLCPP_INFO(logger_, "roll: %f, pitch: %f, yaw: %f, thrust: %u", roll, pitch, yawrate, (unsigned int)thrust);
    cf_.sendHoverSetpoint(vx, vy, yawrate, height);
  }

  void cmd_vel_world_changed(const geometry_msgs::msg::Twist::SharedPtr msg)
  {
    ...
    cf_.sendVelocityWorldSetpoint(x, y, z, yawRate);
  }
...
private:
  ...
  rclcpp::Subscription<geometry_msgs::msg::Twist>::SharedPtr subscription_cmd_vel_hover_;
