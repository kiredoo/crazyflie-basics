...
clear; close all;
bag = ros2bagreader("rosbag2_2025_05_20-22_21_23_0.db3");
% Select messages from the desired topic
% Filter messages by the specific topic
...
colormap2 = colormap_cf2(round(linspace(1, size(colormap_cf2, 1), length(x_positions_3) - 1)), :);
% colormap232 = colormap_cf3(round(linspace(1, size(colormap_cf3, 1), length(x_positions_232) - 1)), :);
colormap233 = colormap_cf4(round(linspace(1, size(colormap_cf4, 1), length(x_positions_233) - 1)), :);
figure;
hold on;

% Plot Crazyfly 1 with gradient
...
%          'Color', colormap232(i, :), 'LineWidth', 2);
% end

% Plot Crazyfly 233 with gradient
for i = 1:(length(x_positions_233) - 1)
    plot(x_positions_233(i:i+1), y_positions_233(i:i+1), ...
         'Color', colormap233(i, :), 'LineWidth', 2);
end

% Plot Crazyfly 2 with gradient
for i = 1:(length(x_positions_3) - 1)
    plot(x_positions_3(i:i+1), y_positions_3(i:i+1), ...
         'Color', colormap2(i, :), 'LineWidth', 2);
end

h1=plot(x_positions_231(end), y_positions_231(end), 'bo', 'MarkerSize', 10, 'DisplayName', 'CF1 Endpoint');
h2=plot(x_positions_3(end), y_positions_3(end), 'kx', 'MarkerSize', 10, 'DisplayName', 'CF2 Endpoint');
% h3=plot(x_positions_232(end), y_positions_232(end), 'gsquare', 'MarkerSize', 10, 'DisplayName', 'CF3 Endpoint');
h3=plot(x_positions_233(end), y_positions_233(end), 'r*', 'MarkerSize', 10, 'DisplayName', 'CF3 Endpoint');
xlabel('X Position');
ylabel('Y Position');
legend([h1;h2;h3],['CF1 Endpoint';'CF2 Endpoint';"CF3 Endpoint"])
