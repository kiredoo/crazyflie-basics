...
#         # Initialize the plot
#         self.fig, self.ax = plt.subplots()
#         self.sc = self.ax.scatter(0.0, 0.0)
#         self.ax.set_xlim(-10, 10)
#         self.ax.set_ylim(-10, 10)
#         plt.ion()
#         plt.show()

#         # Initialize state variables within the class
#         self.xs = np.array([0.0, 0.0, 0.0])
#         self.t = 0.0
...
#         # Debugging statement to check type and value of self.xs
#         print(f"Before rk4: self.xs = {self.xs}, type = {type(self.xs)}")

#         # Check if self.xs is a NumPy array before calling rk4
#         if not isinstance(self.xs, np.ndarray):
#             raise TypeError(f"self.xs should be a NumPy array, but got {type(self.xs)}")

#         # Check self.t
#         print(f"self.t = {self.t}, type = {type(self.t)}")

#         # Perform RK4 calculation
#         dx = rk4(f_unicycle, self.xs, self.t, dt, self.u)
#         self.xs += dx
#         self.t += dt

#         # Debugging statement to check updated value of self.xs
#         print(f"After rk4: self.xs = {self.xs}, type = {type(self.xs)}")
